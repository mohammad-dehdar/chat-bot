...
export const create = <TState>(initializer: StateCreator<TState>): UseBoundStore<TState> => {
  ...
  const setState: SetState<TState> = (partial, replace) => {
    ...
    const nextState = replace ? (nextStatePartial as TState) : ({ ...state, ...nextStatePartial } as TState);
  ...
  const useStore = (<TSelected>(selector?: (state: TState) => TSelected) => {
    ...
    return useSyncExternalStore(subscribe, () => selectorFn(state), () => selectorFn(state));
